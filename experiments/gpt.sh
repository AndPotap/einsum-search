...
CUDA_VISIBLE_DEVICES=0 python train_gpt.py config/train_open_small.py --block_size=${BLOCK_SIZE} --struct=simple_ein_vec_norm --expr=${theta} --layers=all_but_last --d_model=${d_model} --n_layer=${n_layer} --n_head=-1 --d_head=64 --max_iters=${MAX_ITERS} --data_dir=${DATA_DIR} --out_dir=${OUT_DIR} --batch_size=${BATCH_SIZE} --gradient_accumulation_steps=${GRAD_ACCUM} --init_lr=${lr} --wandb_project=${WANDB_PROJ}
done
done
done